...
import type { QNextVerse } from "../qtypes";

type Props = { q: QNextVerse; onGrade: (ok: boolean) => void };

export default function NextVerse({ q, onGrade }: Props) {
  const P = typeof q.prompt === "string" ? { verse: q.prompt } : q.prompt;
  const [sel, setSel] = useState<number | null>(null);
  const [submitted, setSubmitted] = useState(false);
  const options: { text: string; ref?: string }[] = q.payload.options;   // 👈 typed
  ...
  return (
    <div dir="rtl">
      <p className="text-lg font-semibold">{P.verse}</p>
      {P.ref && <p className="text-xs text-slate-500 mb-4">({P.ref})</p>}
      <div className="grid gap-3">
        {options.map((o, i) => (
          <button key={i} onClick={() => setSel(i)} className={cls(i)}>{o.text}</button>
        ))}
      </div>
